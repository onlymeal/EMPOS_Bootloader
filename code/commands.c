...
bool DoHelp(int argc, char **argv)
{
	...
	printf("\t13. game \t: Start Baseball game\n");
	printf("\t15. help \t: Print help message for users to operate this program\n");
	printf("\t16. test \t: Add your code in this function for S/W, H/W test\n");
	...
	printf("\n\t* Use 'help command' to get help on a specific command *\n");
		...
		if (!StrCmp(argv[1], "boot")){
			...
			printf("\tstart Baseball game. Input 4 number, Compare with Random number\n ");
		}else
...
//	u8 mac[6];
	...
	if (!FlashWrite((u32 *)CONFIG_ROM_BASE,macaddr)){
		printf(" Write Failed\n");
		return false;
	}
...
//	u8 *printaddr;
	...
	printf("\n\t* Network Status *\n");
	printf("\tHost      (server) Ethernet : 0x");	PrintEthAddr(hostEther); printf("\n");
	printf("\tHost      (server) IP       : ");	PrintIPAddr(hostIP);	printf("\n");
	printf("\tClient    (target) Ethernet : 0x");	PrintEthAddr(clientEther);	printf("\n");
...
bool DoGame (int argc, char **argv)
{	
	int i, j;
	int destarr[4];
	int inputarr[4];
	char commandline[5];
	int score=0, k=0, two;
	int segcode[10]={0x3f, 0x06, 0x5b, 0x4f, 0x66, 0x6d, 0x7d, 0x07, 0x7f, 0x6f};

	destarr[0]=GetTime()%10;
	printf("/----------Baseball game----------/\n");
	printf("-----------------------------------\n");
	destarr[2]=GetTime()%10;
	printf("     Input 4 Number!!!\n");
	destarr[3]=GetTime()%10;
	GetCommand(&commandline[0]);
	for(i=0;i<4;i++)
		inputarr[i]=commandline[i]-48;
	
	destarr[1]=GetTime()%10;
	for(i=0;i<4;i++)
	{
		if(destarr[i]==inputarr[i])
		{
			two=1;
			for(j=0; j<(7-i);j++)
				two=two*2;
	...
	__REG8(0x10600000)=score;			
	__REG(0x10400000)=(segcode[destarr[0]]<<8) | segcode[destarr[1]];
	__REG(0x10300000)=(segcode[destarr[2]]<<8) | segcode[destarr[3]];
